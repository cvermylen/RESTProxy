...
reply_t* create_response(reply_t* data){
	data->response = (char*)malloc((strlen(resp) +1 ) * sizeof(char));
	strcpy(data->response, resp);
	data->response_length = strlen(resp);
	return data;
}
...
void file_writer(void *params)
{
	...
	fputs(reply->request->buffer, reply->content.file->file);
